...
def render_target_fund():
    ...
    if is_mc_tf:
        # === 상품별 파라미터 설정 ===
        # 적금: 확정금리, 이자소득세 15.4%
        savings_rate = 3.0  # 2026년 2월 기준 시중은행 적금 평균 금리
        savings_tax = 0.154
        
        # 펀드: 변동성 있음, 배당소득세 15.4%
        fund_rate = actual_rate  # 사용자 입력 수익률
        fund_vol = 0.15  # 펀드 변동성
        fund_tax = 0.154
        
        # 변액ETF: 변동성 있음, 비과세 (10년 이상 유지 시)
        etf_rate = actual_rate  # 동일 기대수익률
