...
with st.sidebar:
    ...
    with c_sync:
        if st.button("📋 샘플", help="샘플 데이터를 불러옵니다 (데모용).", use_container_width=True):
...
def render_real_estate():
    ...
    with tab2:
        ...
        with col_result:
            ...
            c1.metric("재산세\n(본세+부가세)", f"{f_w(prop_tax)} 원", delta=f"본세 {f_w(prop_tax_base)}")
            c2.metric(f"종부세\n(세율 {j_rate_desc})", f"{f_w(j_tax_final)} 원", f"-{int(deduction_rate*100)}% 공제" if deduction_rate > 0 else None)
            c3.metric("종부 농어촌특별세\n(종부세의 20%)", f"{f_w(jongbu_rural_tax_final)} 원")
            c4.metric("총 보유세 합계", f"{f_w(total_hold_tax)} 원")
    ...
    with tab3:
        ...
        if not st.session_state.presentation_mode:
            with col_input:
                ...
                if st.button("🔗 취득세 입력값 동기화", help="취득세 탭에 입력한 가격·면적 정보를 양도세 취득가로 가져옵니다.", use_container_width=True):
        ...
        with col_result:
            ...
            c3.metric("세후 실수령액", f"{f_w(max(0, sell_p - total_yang_tax))} 원", delta_color="normal")

            if _short_rate is not None:
                st.error(f"🚨 {_short_desc} — 보유기간 {h_years}년 미만으로 중과세율 적용!")
            if surcharge_rate > 0:
