    ...
    def get_refund_curve(ptype, period):
        ...
        if "메트라이프" in ptype:
            if period == 5:
                ...
                # 10년: 2차 보너스(11.0%) 합산 → 124.86%
                base = [0, 35, 60, 78, 102.5, 106, 110, 114, 119, 124.86]
                last = 124.86
                for _ in range(11):
                    last *= 1.025
                    base.append(last)
                ...
                # 10년: 2차 보너스(15.9%) 합산 → 124.96%
                base = [0, 25, 45, 62, 73, 82, 104.2, 110, 117, 124.96]
                last = 124.96
                for _ in range(11):
                    last *= 1.025
                    base.append(last)
...
""")
        ...
        scenario_html += "<div style='flex:1; min-width:120px; padding:14px; border-radius:14px; text-align:center; box-shadow:0 4px 16px rgba(100,116,139,0.2); background:linear-gradient(135deg,#334155,#64748b); border:none;'>"
        scenario_html += "<div style='font-size:1.6rem; margin-bottom:6px;'>📉</div>"
        scenario_html += "<div style='font-size:0.85rem; font-weight:bold; color:rgba(255,255,255,0.9);'>하락(비관)</div>"
        scenario_html += "<div style='font-size:0.75rem; color:rgba(255,255,255,0.6); margin-bottom:8px; font-family:Consolas,monospace;'>@" + rate_low_str + "원/$</div>"
        scenario_html += "<div style='font-size:1.1rem; font-weight:800; color:#ffffff; margin-bottom:4px;'>" + val_low_str + "만원</div>"
        scenario_html += "<div style='font-size:0.8rem; font-weight:700; background:rgba(255,255,255,0.15); padding:3px 6px; border-radius:5px; display:inline-block; color:#ffffff;'>" + f"{profit_low_num:+.1f}%<br>({f_w(p_low/10000)}만)" + "</div></div>"

        scenario_html += "<div style='flex:1; min-width:120px; padding:14px; border-radius:14px; text-align:center; box-shadow:0 4px 16px rgba(59,130,246,0.25); background:linear-gradient(135deg,#1e40af,#3b82f6); border:none;'>"
        scenario_html += "<div style='font-size:1.6rem; margin-bottom:6px;'>☁️</div>"
        scenario_html += "<div style='font-size:0.85rem; font-weight:bold; color:rgba(255,255,255,0.9);'>보합(중립)</div>"
        scenario_html += "<div style='font-size:0.75rem; color:rgba(255,255,255,0.6); margin-bottom:8px; font-family:Consolas,monospace;'>@" + rate_mid_str + "원/$</div>"
        scenario_html += "<div style='font-size:1.1rem; font-weight:800; color:#ffffff; margin-bottom:4px;'>" + val_mid_str + "만원</div>"
        scenario_html += "<div style='font-size:0.8rem; font-weight:700; background:rgba(255,255,255,0.15); padding:3px 6px; border-radius:5px; display:inline-block; color:#ffffff;'>" + f"{profit_mid_num:+.1f}%<br>({f_w(p_mid/10000)}만)" + "</div></div>"

        scenario_html += "<div style='flex:1; min-width:120px; padding:14px; border-radius:14px; text-align:center; box-shadow:0 4px 16px rgba(239,68,68,0.25); background:linear-gradient(135deg,#991b1b,#ef4444); border:none;'>"
        scenario_html += "<div style='font-size:1.6rem; margin-bottom:6px;'>☀️</div>"
        scenario_html += "<div style='font-size:0.85rem; font-weight:bold; color:rgba(255,255,255,0.9);'>상승(낙관)</div>"
        scenario_html += "<div style='font-size:0.75rem; color:rgba(255,255,255,0.6); margin-bottom:8px; font-family:Consolas,monospace;'>@" + rate_high_str + "원/$</div>"
        scenario_html += "<div style='font-size:1.1rem; font-weight:800; color:#ffffff; margin-bottom:4px;'>" + val_high_str + "만원</div>"
        scenario_html += "<div style='font-size:0.8rem; font-weight:700; background:rgba(255,255,255,0.15); padding:3px 6px; border-radius:5px; display:inline-block; color:#ffffff;'>" + f"{profit_high_num:+.1f}%<br>({f_w(p_high/10000)}만)" + "</div></div>"
        scenario_html += "</div>"
